...
- [ ] Confirm Go version: Echo **v5.3.1** needs **Go ≥ 1.25**. Below that, stay on **v4** (`github.com/labstack/echo/v4` + `echo.Context` + `middleware.Logger()`). Ideas are the same; signatures differ.
- [ ] Prefer pin: `go get github.com/labstack/echo/v5@v5.3.1` (not `@latest` on session day).
- [ ] Tabs open: [Custom Middleware cookbook](https://echo.labstack.com/cookbook/middleware/), [IP Address guide](https://echo.labstack.com/guide/ip-address/).
- [ ] Throwaway demo folder ready (`~/acs4210-day5-middleware`).
...
**Pulse check 1 (≤60s):** Which layer owns authz for “can this user buy this SKU?” — middleware or handler/service? Expected: **handler/service** (middleware can do a thin authn gate; SKU rules stay out of the hallway).

### The onion — Echo request lifecycle (~9 min)
...
### Built-ins you’ll actually touch (~5 min)
...
### Live skeleton — custom middleware (~7 min)
...
### IP reality check — fuel for Activity II (~10 min)
...
### Bridge into practice (~2 min)

Activity I: get the hallway compiling — Recover, RequestID, one custom header middleware, `/whoami` printing RealIP. Activity II: decide inside/outside and name the traps. Lab: harden one sharp edge.

## In Class Activity I (20 min)

**Goal:** A running Echo server with real middleware registration.

**Artifact:** a folder with `main.go` that boots on `:1323`.
...
2. Add Echo v5 (pin **v5.3.1**; needs **Go ≥ 1.25** — otherwise stay on v4 and swap imports as noted in Overview):
...
**Goal:** Custom middleware (or a helper used by middleware) that labels a request as inside/outside.
...
**Visible checkpoint:** two curls — normal local (`inside` true) vs a forged header that still follows **your documented rules**.
...
1. Keep Activity I’s module. Set `e.IPExtractor = echo.ExtractIPDirect()`.
...
Pick **one** stretch; keep `/whoami` working.
...
3. **Order demo** — add two tiny middlewares that log enter/leave; paste the order proving outer→inner→handler→inner→outer.

### Stretch Challenges

1. What breaks if you trust the leftmost `X-Forwarded-For` hop from a random client?
2. Why register `Recover` outermost by default — and when would you put the request logger outside it instead?
3. When would you *not* put a check in middleware?
...
Every time you see an `e.Use`, ask “why this layer?”
